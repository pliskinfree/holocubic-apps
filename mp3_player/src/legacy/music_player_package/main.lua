...
MUSIC_PLAYER_APP = {
  ...
  DEMO_ICON_PATH = "/sd/cogwheel.png",
...
<main>
  <header>
    ...
    <div class="actions"><div class="volumeBox"><label>音量</label><input id="mainVolume" type="range" min="0" max="1.2" step="0.01"><span id="mainVolumeVal">0.60</span></div><button class="btn" id="refreshBtn">刷新</button><button class="btn primary" id="eqBtn">EQ</button></div>
  </header>
  <div class="grid">
...
local function demo_icon_src()
  if file and file.exists and file.exists(APP.DEMO_ICON_PATH) then
    return sd_to_lv(APP.DEMO_ICON_PATH), true
  end
  if not file or not file.exists then
    return sd_to_lv(APP.DEMO_ICON_PATH), true
  end
  return asset("disc.png"), false
end
...
local function viper_pcm_push(ctx, data)
  ...
  ctx.buf:from_string(data, ctx.write_pos, 0, first)
  if len > first then
    ctx.buf:from_string(data, 0, first, len - first)
  end
...
local function viper_pcm_pop(ctx, max_bytes)
  ...
  local out = ctx.buf:to_string(ctx.read_pos, first)
  if len > first then
    out = out .. ctx.buf:to_string(0, len - first)
  end
...
local function start_native_i2s_play()
  ...
  local ok, err = pcall(APP.audio.i2s_play_start, {
  ...
  if not ok then
    error("audio.i2s_play_start failed: " .. tostring(err))
  end
  return true
end
...
local function should_finish_track()
  local dur = tonumber(S.duration_ms) or 0
  if dur <= 0 then return true end
  return play_position_ms() >= math.max(0, dur - 800)
end
...
local function load_ui_module()
  ...
  APP._clamp = clamp
  APP._asset = asset
  APP._demo_icon_src = demo_icon_src
  ...
  if not (APP._ui_load_font and APP._ui_release_fonts and APP._ui_build and APP._ui_render and APP._ui_bind_touch) then
